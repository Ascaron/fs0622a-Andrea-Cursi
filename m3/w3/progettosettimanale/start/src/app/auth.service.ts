...
import { AuthResponse, LoginRequest, Movie, RegisterRequest } from './interface/auth-response';
import {map} from 'rxjs/operators';
import { BehaviorSubject } from 'rxjs';
import { Favourites } from './interface/auth-response';
...
export class AuthService {
  ...
  recuperaPreferiti() {
    ...
    return this.http.get<Favourites[]>(`http://localhost:4201/favorites?userId=${user}`, {
      headers: {
        "Authorization": "Bearer " + token
      }
    }).pipe(tap((res) => {
  ...
  addPreferiti(movieId:number){
    ...
    return this.http.post<Favourites>(`http://localhost:4201/favorites`, body,  {
      headers: {
        "Authorization": "Bearer " + token
      }
    })
